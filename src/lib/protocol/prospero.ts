...
const TEAM_ROLE_LABELS: Record<string, string> = {
  ...
  librarian: "Information specialist / librarian",
  consumer: "Patient or public representative",
};

export function formatTeamMembersForProspero(members: TeamMember[] = []): string {
  return members
    .map((member) => {
      const role = TEAM_ROLE_LABELS[member.role] ?? member.role.replace(/_/g, " ");
      const parts = [
        member.name.trim(),
        role,
        member.affiliation?.trim() ? `Affiliation: ${member.affiliation.trim()}` : "",
        member.email?.trim() ? `Email: ${member.email.trim()}` : "",
        member.country?.trim() ? `Country: ${member.country.trim()}` : "",
        member.contribution?.trim() ? `Role in review: ${member.contribution.trim()}` : "",
        member.conflictOfInterest?.trim()
          ? `Conflict of interest: ${member.conflictOfInterest.trim()}`
...
export function buildProsperoDraftHtml(review: Review): string {
  ...
  const sections = (Object.keys(PROSPERO_SECTION_LABELS) as ProtocolSectionId[]).map((section) => {
    ...
    return `<h2>${escapeHtml(PROSPERO_SECTION_LABELS[section])}</h2>${fields.map((field) => {
      const teamText = field.id === "teamMembers"
        ? formatTeamMembersForProspero(workspace.team?.members ?? [])
        : "";
      const answer = teamText || workspace.answers[field.id] || "";
      return `<h3>${escapeHtml(field.prompt)}</h3><p>${escapeHtml(answer) || "<em>Not completed</em>"}</p>`;
    }).join("")}`;
  }).join("");
